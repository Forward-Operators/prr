...
"""
    ...
    def test_basic_services_model_list_with_some_options(self):
        config = PromptConfig()
        config.load_from_config_contents(
            """
prompt:
...
"""
        ...
        for service_name in services:
            assert (
                config.option_for_service(service_name, "temperature")
                == ModelOptions.DEFAULT_OPTIONS["temperature"]
            )
            assert (
                config.option_for_service(service_name, "max_tokens")
                == ModelOptions.DEFAULT_OPTIONS["max_tokens"]
            )
            assert config.option_for_service(service_name, "top_k") == -1.337
            assert (
                config.option_for_service(service_name, "top_p")
                == ModelOptions.DEFAULT_OPTIONS["top_p"]
            )

    def test_basic_services_model_list_with_no_options(self):
        config = PromptConfig()
        config.load_from_config_contents(
            """
prompt:
...
"""
        ...
        for service_name in services:
            ...
            assert (
                config.option_for_service(service_name, "top_k")
                == ModelOptions.DEFAULT_OPTIONS["top_k"]
            )
            assert (
                config.option_for_service(service_name, "top_p")
                == ModelOptions.DEFAULT_OPTIONS["top_p"]
