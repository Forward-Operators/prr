...
class RunPromptCommand():
  ...
  def print_run_results(self, result, run_save_directory):
    ...
    if self.args['quiet']:
      ...
      if self.args['abbrev']:
        self.console.log("Prompt:      " + "[yellow]" + request.prompt_text(25).strip() + f"[/yellow] ({len(request.prompt_text())} chars)")
        self.console.log("Completion:  " + "[green]" + response.response_abbrev(25).strip() + f"[/green] ({len(response.response_content)} chars)")
      else:
        self.console.log(Panel('[yellow]' + request.prompt_text().strip() + '[/yellow]'))
        self.console.log(Panel('[green]' + response.response_content.strip() + '[/green]'))
  ...
  def run_prompt_on_service(self, service_name, save=False):
    ...
    with self.console.status(f":robot: [bold green]{service_name}[/bold green]") as status:
  ...
  def load_prompt_for_path(self):
    ...
    if not os.path.exists(prompt_path) or not os.access(prompt_path, os.R_OK):
      self.console.log(f":x: Prompt file {prompt_path} is not accessible, giving up.")
      exit(-1)
  ...
  def run_prompt(self):
    ...
    if services_to_run == []:
      if self.args['service']:
        ...
        self.console.log(f":racing_car:  Running service {self.args['service']} with default options.")
      else:
        self.console.log(f":x: No services configured for prompt {self.args['prompt_path']}, nor given in command-line. Not even in .env!")
        exit(-1)
    else:
